<?php 
include '/var/www/html/menu/menu.php';
require '/var/www/html/menu/db.php';

$query = $_GET['query'];
console_log($query);
//검색결과 개수를 알기위해 검색어($query)만 조건으로 걸어 select
$sql = queryResult("SELECT *FROM review_note WHERE title LIKE '%$query%' OR content LIKE '%$query%'");
$totalReviewSearchNum = mysqli_num_rows($sql); //검색결과 개수
...
$totalPageNum = ceil($totalReviewSearchNum/$pageScale); //총 페이지수 = 총 게시물수/페이지당 게시글 수
...
//검색 결과를 출력하기 위해 검색어 + 글번호를 거꾸로(최신순) 정렬 + 각 페이지당 시작할 글번호, 보여줄 글 개수를 조건으로 select
$sqlLimit = queryResult("SELECT *FROM review_note WHERE title LIKE '%$query%' OR content LIKE '%$query%' 
                        ORDER BY review_num DESC LIMIT $pageStart, $pageScale");
?>

<div class="dash-content">
    <div class="container-fluid" id="reviewSearchResult">
	<div class="row">
        <div><?=$totalReviewSearchNum?>개의 검색 결과가 있습니다.</div>
    </div>
            ...
            <div class="bg-white p-3 rounded mt-3" style="width:1080px;">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <img src="img-uploads/gameofthrones.jpg" class="card-img" style="position: absolute; width:95%; height:95%;"/>
        ...
        <?php 
            ...
            if($page <= 1){
                ...
                echo "<li class=\"page-item\">
                        <a class=\"page-link\" href=\"?query=$query&page=1\"><<</a>
                    </li>";
            }
            ...
            // ?뒤에 전달할 변수명 = 값 을 붙여 링크해준다
                ...
                $pre = $page - 1;
                echo "<li class=\"page-item\">
                        <a class=\"page-link\" href=\"?query=$query&page=$pre\">Previous</a>
                    </li>";
            }
            // 가운데 숫자 블록 부분
            for($i=$blockStart; $i<=$blockEnd; $i++){ 
                ...
                if($page == $i){ 
                  ...
                  echo "<li class=\"page-item\">
                  <a class=\"page-link\" href=\"?query=$query&page=$i\">$i</a></li>";
                }
              }
              if($currentBlock >= $totalBlockNum){ 
                ...
                $next = $page + 1;
                echo "<li class=\"page-item\">
                        <a class=\"page-link\" href=\"?query=$query&page=$next\">Next</a></li>";
              }
            //   페이지가 총 페이지수보다 많거나 같으면 마지막버튼 비활성화
              if($page >= $totalPageNum){
                ...
                echo "<li class=\"page-item\">
                        <a class=\"page-link\" href=\"?query=$query&page=$totalPageNum\">>></a>
                    </li>";
              }
              ?>
                </ul>
