...
// 장르 아이디로 리스트를 불러온다
  ...
  data: { "api_key": "d579af00349a9e85a6a32ff41c93ad8c", "page": "1", "with_genres": "35" },
  dataType: "json"
})
  .done(function( comedyResult ) {
    for (i = 0; i < comedyResult.results.length; i++) {
      var image = comedyResult.results[i].poster_path == null ? "/menu/img/no-image-icon-23485.png" : "https://image.tmdb.org/t/p/w185/" + comedyResult.results[i].poster_path;
      var name = comedyResult.results[i].name;
      var firstAirDate = comedyResult.results[i].first_air_date;
      var tv_id = comedyResult.results[i].id;
