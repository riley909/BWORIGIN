...
// 장르 아이디로 리스트를 불러온다
  ...
  data: { "api_key": "d579af00349a9e85a6a32ff41c93ad8c", "page": "1", "with_genres": "99" },
  dataType: "json"
})
  .done(function( documentaryResult ) {
    for (i = 0; i < documentaryResult.results.length; i++) {
      var image = documentaryResult.results[i].poster_path == null ? "/menu/img/no-image-icon-23485.png" : "https://image.tmdb.org/t/p/w185/" + documentaryResult.results[i].poster_path;
      var name = documentaryResult.results[i].name;
      var firstAirDate = documentaryResult.results[i].first_air_date;
      var tv_id = documentaryResult.results[i].id;
