				...
				<section id="main">
					<div class="container">
						<div class="row">
								...
								<!-- Sidebar -->
									<section class="box">
										...
										<footer>
											...
											<a href="" class="icon solid fa-cog fa-lg" style="color:#5d5d5d;"></a>
										</footer>
									</section>
								<?php } ?>
							...
							$authoridcheck = $_SESSION['session_id'];

							$sql = "SELECT *FROM blog WHERE authorid='$authoridcheck'";
							$res = $conn->query($sql);
							...
							// 한페이지당 출력될 부분
							// order by (컬럼명) desc(내림차순) limit(개수제한) $currentpagenum(시작위치=이 페이지부터..) 4(반환개수=게시물 몇개 보여줄지)
							$sql2 = "select *from blog where authorid='$authoridcheck' order by postnum desc limit $currentpagenum, 4";
							...
							<div class="col-9 col-12-medium imp-medium">
								...
								<article class="box post">
										...
										<div style="float:right;">
											<table >
												...
												<!-- 삭제버튼 -->
												<?php if($_SESSION['session_id'] == $row['authorid']){?>
												<th><a href="/stdbhs/templ/postdelete.php?postnum='.$row['postnum'].'" style="display: inline-block; text-decoration: none; border-radius: 5px; background: #9eb86d; color: #fff; padding: 0.2em;">삭제</a></th>
											<?php } ?>
								...
								<div style="display:-ms-flexbox; display:flex; -ms-flex-wrap:wrap; flex-wrap:wrap; margin-right:-15px; margin-left:-15px; -ms-flex-pack:center; justify-content:center; text-decoration:none; color:#779442">
										...
										if($pagination > 1){
											echo "<a href='/stdbhs/templ/myblog.php?pagination=$before&page=$before2'>&laquo;</a>";
										}

										for($i = $pagination*5-4; $i <= $pagination*5; $i++){
											if($i <= $totalpagenum){
												echo "<a href='/stdbhs/templ/myblog.php?pagination=$pagination&page=$i'>[$i]</a>";
											}else {
												break;
											}
										}
										...
										if($pagination < $totalblocknum){
											echo "<a href='/stdbhs/templ/myblog.php?pagination=$after&page=$after2'>&raquo;</a>";
										}
										 ?>
								</div>
